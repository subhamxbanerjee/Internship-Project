...
public class IncidentController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<IncidentResponse> getIncident(@PathVariable("id") Long id, Authentication auth) {
    ...
    public ResponseEntity<IncidentResponse> assignEmployee(@PathVariable("id") Long id, @RequestBody IncidentRequest request, Authentication auth) {
        Incident incident = incidentService.assignEmployee(id, request.getAssignedTo(), auth.getName());
        return ResponseEntity.ok(IncidentResponse.fromEntity(incident));
    }

    @PutMapping("/{id}/status")
    public ResponseEntity<IncidentResponse> updateStatus(@PathVariable("id") Long id, @RequestBody IncidentRequest request, Authentication auth) {
        Incident incident = incidentService.updateStatus(id, request.getStatus(), auth.getName());
        return ResponseEntity.ok(IncidentResponse.fromEntity(incident));
    }

    @PutMapping("/{id}/priority")
    public ResponseEntity<IncidentResponse> updatePriority(@PathVariable("id") Long id, @RequestBody IncidentRequest request, Authentication auth) {
        Incident incident = incidentService.updatePriority(id, request.getPriority(), auth.getName());
        return ResponseEntity.ok(IncidentResponse.fromEntity(incident));
    }

    @PutMapping("/{id}/comment")
    public ResponseEntity<IncidentResponse> addComment(@PathVariable("id") Long id, @RequestBody IncidentRequest request, Authentication auth) {
        Incident incident = incidentService.addComment(id, request.getComment(), auth.getName());
        return ResponseEntity.ok(IncidentResponse.fromEntity(incident));
    }

    @PutMapping("/{id}/close")
    public ResponseEntity<IncidentResponse> closeIncident(@PathVariable("id") Long id, Authentication auth) {
        Incident incident = incidentService.closeIncident(id, auth.getName());
        return ResponseEntity.ok(IncidentResponse.fromEntity(incident));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Map<String, String>> deleteIncident(@PathVariable("id") Long id, Authentication auth) {
